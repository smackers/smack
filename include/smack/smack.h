 ...
 * 3. heap addresses are allocated in a strictly increasing fashion
 * 4. freed (heap) addresses are never reallocated
 ...
 *
 * 1. Currently SMACK does not preceisely calculate procedures' `modifies`
 *    specifications. Instead, the `modifies` specifications of every procedure
 *    (besides those defined by `__SMACK_top_decl`, e.g., in this file)
 *    blindly include the `$M.i` variable, for each (calculated) memory
 *    region `i`, as well as the `$CurrAddr` and `$Alloc` variables.
 *
 * 2. Consequently, Boogie global variable declarations may only be added or
 *    removed if the `modifies` specifications generated by SMACK are discarded
 *    (or if the corresponding change is made in SMACK's source code).
 */
