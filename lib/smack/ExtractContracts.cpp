...
namespace {
  ...
  std::vector<Function*> getContractExprs(Function *F) {
    std::vector<Function*> Fs;

    // TODO call llvm::CloneFunctionInto(...) for each contract invocation in F
...
bool ExtractContracts::runOnModule(Module &M) {
  ...
  auto *requiresFn = M.getFunction(Naming::CONTRACT_REQUIRES);
  auto *ensuresFn = M.getFunction(Naming::CONTRACT_ENSURES);
  auto *invariantFn = M.getFunction(Naming::CONTRACT_INVARIANT);
  ...
  for (auto F : Fs) {
    ...
    if (contractBlocks.size()) {
      ...
      for (auto I : Is) {
        IRBuilder<> Builder(I);
        for (auto CF : getContractExprs(newF)) {
          std::vector<Value*> ArgsV;
          for (auto &Arg : I->arg_operands())
            ArgsV.push_back(Arg);
          auto *E = Builder.CreateCall(CF, ArgsV);

          // TODO either requires or ensures

          Builder.CreateCall(requiresFn, {E});
        }
        I->eraseFromParent();
      }

      DEBUG(errs() << "function " << F->getName() << " after contract extraction: " << *F << "\n");
      DEBUG(errs() << "added function " << *newF);
      ...
      for (auto I : Is) {
        ...
        for (auto CF : getContractExprs(newF)) {
          ...
          for (auto &Arg : I->arg_operands())
            ArgsV.push_back(Arg);
          auto *E = Builder.CreateCall(CF, ArgsV);
          Builder.CreateCall(invariantFn, {E});
        }
        I->eraseFromParent();
      }
