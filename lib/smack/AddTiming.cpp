...
static bool isAutogeneratedSmackFunction(Function& F) {
  auto name = F.getName();
  return
    (begins_with("__SMACK", name) ||
     begins_with("__VERIFIER", name));
}
...
AddTiming::runOnFunction(Function &F) {
 ...
 if (isAutogeneratedSmackFunction(F)) {
   //No need to annotate autogenerated functions like __VERIFIER_assume or __SMACK_dummy
   return false;
 }
 ...
 for (Function::iterator B = F.begin(), BE = F.end(); B != BE; ++B) {
   for (BasicBlock::iterator it = B->begin(), e = B->end(); it != e; ++it) {
     Instruction *Inst = &*it;
     //Add the naming metadata first, so we don't get unnecessary metadata in the print
...
void
AddTiming::addTimingMetadata(Instruction* Inst) const {
...
void
AddTiming::addNamingMetadata(Instruction* Inst) const {
...
void AddTiming::addMetadata(Instruction *Inst, const std::string &name, const std::string& value) const {
