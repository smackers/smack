...
std::string getMapTypeName(std::string elemType) {
  return "[ref] " + elemType;
}
...
std::string makeIntVarName(unsigned i) {
  return makeVarName(i, "i");
}

// make var names such as p, p1
std::string makePtrVarName(unsigned i) {
  return makeVarName(i, "p");
}

// make var names such as f, f1
std::string makeFpVarName(unsigned i) {
  return makeVarName(i, "f");
}

// make var names such as M, M1
std::string makeMapVarName(unsigned i) {
  return makeVarName(i, "M");
}

std::string makeRMODEVarName() {
...
const Expr* makePtrVarExpr(unsigned i) {
  return Expr::id(makePtrVarName(i));
}

const Expr* makeFpVarExpr(unsigned i) {
  return Expr::id(makeFpVarName(i));
}

const Expr* makeMapVarExpr(unsigned i) {
  return Expr::id(makeMapVarName(i));
}
...
std::list<Binding> makeIntVars(unsigned numVars, std::string type) {
  return makeListOfElems<Binding>(numVars,
    [&](unsigned i) -> Binding { return {makeIntVarName(i), type}; });
}
...
std::list<Binding> makePtrVars(unsigned numVars) {
  return makeListOfElems<Binding>(numVars,
    [&](unsigned i) ->
      Binding { return {makePtrVarName(i), Naming::PTR_TYPE}; });
}
...
std::list<Binding> makeFpVars(unsigned numVars, unsigned bitWidth) {
  return makeListOfElems<Binding>(numVars,
    [&](unsigned i) ->
      Binding { return {makeFpVarName(i), getFpTypeName(bitWidth)}; });
}
...
std::list<Binding> makeMapVars(unsigned numVars, std::string elemType) {
  return makeListOfElems<Binding>(numVars,
    [&](unsigned i) ->
      Binding { return {makeMapVarName(i), getMapTypeName(elemType)}; });
}
...
FuncDecl* uninterpretedOp(std::string baseName,
  ...
  std::string retType) {
  return Decl::function(indexedName(baseName, nameArgs), args, retType,
    nullptr, {});
}
...
const std::vector<unsigned> FpOpGen::FP_BIT_WIDTHS {
  16, 32, 64, 80
};
...
const Expr* Prelude::mapSelExpr (unsigned idx) {
  ...
  auto idxExpr = idx?
    Expr::fn(indexedName("$add", {Naming::PTR_TYPE}),
        ptrVar, rep.pointerLit((unsigned long long)idx)) : ptrVar;
...
FuncDecl* Prelude::safeStore(Binding elemBinding) {
  ...
  return inlinedOp("$store", {elemType},
    {makeMapVars(1,elemType).front(),
    makePtrVars(1).front(),
    elemBinding},
    getMapTypeName(elemType),
...
struct IntOpGen::IntArithOp : public IntOp {
  IntArithOp(std::string opName, unsigned arity, Op* intOp, Op* bvOp, bool alsoUsedByPtr) :
  ...
  FuncDecl* getIntFunc(unsigned size) const {
    ...
    if (auto bop = dyn_cast<BuiltinOp<IntOp::attrT>>(intOp))
      ...
      return inlinedOp(name, {type}, makeIntVars(arity, type), type, ((IntOp::exprT)iop->func)(size));
    else
  ...
  FuncDecl* getBvFunc(unsigned size) const {
    ...
    if (auto bop = dyn_cast<BuiltinOp<IntOp::attrT>>(bvOp))
      ...
      //e.g.: function {:inline} $smin.bv32(i1: bv32, i2: bv32) returns (bv32) { if $slt.bv32.bool(i1, i2) then i1 else i2 }
      return inlinedOp(name, {type}, makeIntVars(arity, type), type, ((IntOp::exprT)iop->func)(size));
    else
  ...
  FuncsT getFuncs(unsigned size) const override {
    FuncsT funcs;
    if (!SmackOptions::BitPrecise || (!SmackOptions::BitPrecisePointers && alsoUsedByPtr))
...
void IntOpGen::generateArithOps(std::stringstream& s) const {
  ...
  const std::vector<IntOpGen::IntArithOp> intArithOpTable {
    {"add", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Plus>),
      bvBuiltinOp, true},
    {"sub", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Minus>),
      bvBuiltinOp, true},
    {"mul", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Times>),
      bvBuiltinOp, true},
    ...
    {"smin", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intMinMaxExpr<true>),
        new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::bvMinMaxExpr<true, true>), false},
    {"smax", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intMinMaxExpr<false>),
        new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::bvMinMaxExpr<true, false>), false},
    {"umin", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intMinMaxExpr<true>),
        new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::bvMinMaxExpr<false, true>), false},
    {"umax", 2, new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intMinMaxExpr<false>),
        new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::bvMinMaxExpr<false, false>), false}
  };
  ...
  if (!SmackOptions::BitPrecise) {
    ...
    for (unsigned i = 0; i < 2; ++i) {
      for (unsigned j = 0; j < 2; ++j) {
        ...
        s << Decl::axiom(Expr::eq(Expr::fn(indexedName("$or", {getIntTypeName(1)}),
                Expr::lit(i), Expr::lit(j)), Expr::lit(i | j))) << "\n";
        s << Decl::axiom(Expr::eq(Expr::fn(indexedName("$xor", {getIntTypeName(1)}),
                Expr::lit(i), Expr::lit(j)), Expr::lit(i ^ j))) << "\n";
      }
    }
...
struct IntOpGen::IntPred : public IntOp {
  ...
  std::pair<FuncDecl*, FuncDecl*> getIntFuncs(unsigned size) const {
    ...
    //        { (i1 <= i2) }
    auto compFunc = inlinedOp(name, {type, Naming::BOOL_TYPE},
        makeIntVars(2, type), Naming::BOOL_TYPE, ((IntOp::exprT)iop->func)(size));
    //e.g.: function {:inline} $ule.i1(i1: i1, i2: i1) returns (i1)
    //        { if $ule.i1.bool(i1, i2) then 1 else 0 }
    auto predFunc = inlinedOp(name, {type}, makeIntVars(2, type), getIntTypeName(1),
        Expr::if_then_else(
...
void IntOpGen::generatePreds(std::stringstream& s) const {
  ...
  const auto geInlinedOp = new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Gte>);
  const auto gtInlinedOp = new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Gt>);
  const auto eqInlinedOp = new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Eq>);
  const auto neInlinedOp = new InlinedOp<IntOp::exprT>(IntOpGen::IntArithOp::intArithExpr<BinExpr::Neq>);
  const std::vector<IntPred> intPredTable {
    {"ule", leInlinedOp , bvBuiltinOp},
    {"ult", ltInlinedOp, bvBuiltinOp},
    {"uge", geInlinedOp, bvBuiltinOp},
    {"ugt", gtInlinedOp, bvBuiltinOp},
    {"sle", leInlinedOp, bvBuiltinOp},
    {"slt", ltInlinedOp, bvBuiltinOp},
    {"sge", geInlinedOp, bvBuiltinOp},
    {"sgt", gtInlinedOp, bvBuiltinOp},
    {"eq", eqInlinedOp, eqInlinedOp},
    {"ne", neInlinedOp, neInlinedOp}};
...
struct IntOpGen::IntConv {
  ...
  FuncDecl* getBvFunc(unsigned size1, unsigned size2) const {
    ...
    if (auto bop = dyn_cast<BuiltinOp<attrT>>(bvOp)) {
      //e.g.: function {:bvbuiltin (_ sign_extend 8)} $sext.bv8.bv16(i: bv8) returns (bv16);
...
void IntOpGen::generateConvOps(std::stringstream& s) const {
  ...
  const auto builtinZext = new BuiltinOp<IntConv::attrT>(IntConv::extAttr<false>);
...
void IntOpGen::generateBvIntConvs(std::stringstream& s) const {
  ...
  std::string bt = "bv" + b;
  std::string it = "i" + b;
  s << Decl::function(indexedName("$int2bv",{ptrSize}),
    {{"i",it}}, bt, nullptr, {makeBuiltinAttr("(_ int2bv " + b + ")")})
    << "\n";
  if (SmackOptions::BitPrecise && !SmackOptions::BitPrecisePointers) {
    s << Decl::function(indexedName("$bv2uint",{ptrSize}),
    ...
    s << Decl::function(indexedName("$bv2int",{ptrSize}),
      {{"i",bt}}, it, Expr::cond(
...
void ConstDeclGen::generatePtrConstant(unsigned val, std::stringstream& s) const {
  std::string ptrConst = indexedName("$"+std::to_string(val), {Naming::PTR_TYPE});
...
void ConstDeclGen::generateIntConstant(unsigned val, std::stringstream& s) const {
...
void MemDeclGen::generateAddrBoundsAndPred(std::stringstream& s) const {
  ...
  s << Decl::function(Naming::EXTERNAL_ADDR, makePtrVars(1), Naming::BOOL_TYPE,
...
void PtrOpGen::generatePtrNumConvs(std::stringstream& s) const {
  ...
  for (unsigned i = 8; i <= 64; i <<= 1) {
    s << Decl::function(indexedName("$p2i", {Naming::PTR_TYPE,
      ...
      << "\n";
    s << Decl::function(indexedName("$i2p", {prelude.rep.intType(i),
        Naming::PTR_TYPE}), {{"i",prelude.rep.intType(i)}}, Naming::PTR_TYPE,
...
void PtrOpGen::generatePreds(std::stringstream& s) const {
  ...
  for (auto pred : predicates) {
    s << Decl::function(indexedName(pred,{Naming::PTR_TYPE}),
      ...
      Expr::cond(
        Expr::fn(indexedName(pred,{prelude.rep.pointerType(),Naming::BOOL_TYPE}),
...
struct FpOpGen::FpArithOp : public FpOp {
  ...
  FpArithOp(std::string opName, unsigned arity, bool rMode) :
    FpOp(opName, arity, new BuiltinOp<FpOp::attrT>(fpAttrFunc)), rMode(rMode) {}
...
struct FpOpGen::FpPred : public FpOp {
  typedef const Expr* (*exprT)(std::string, const Expr* a1, const Expr* a2, unsigned bitWidth);
  bool llvmBuiltin;
  ...
  FuncDecl* getModeledFpFunc(unsigned bitWidth) const override {
    ...
    if (auto bop = dyn_cast<BuiltinOp<FpOp::attrT>>(op)) {
      ...
      //e.g: function {:builtin "fp.isNormal"} $isnormal.bvhalf.bool(i: bvhalf) returns (bool);
      ...
      //e.g.:  function {:inline} $ford.bvhalf.bool(f1:bvhalf, f2:bvhalf) returns (bool) {!$funo.bvhalf.bool(f1,f2)}
      return inlinedOp(name, {type, Naming::BOOL_TYPE},
          makeFpVars(arity, bitWidth), Naming::BOOL_TYPE, ((exprT)iop->func)(opName, a1, a2, bitWidth));
    } else
    ...
    for (auto srcBw : FP_BIT_WIDTHS) {
      for (auto desBw : FP_BIT_WIDTHS) {
        if (srcBw != desBw) {
          ...
          std::string attr = "(_ to_fp " + std::to_string(exp) + " " + std::to_string(sig) + ")";
...
struct FpOpGen::FpIntConv {
  ...
  FuncDecl* getUninterpretedFpIntCast(unsigned fpBw, unsigned intBw, bool rMode) const {
  ...
  static const Attr* fpIntAttr(unsigned fpBw, unsigned intBw) {
    std::string signLetter = SIGN? "s" : "u";
    std::string attr = F2I? ("(_ fp.to_" + signLetter + "bv " + std::to_string(intBw) + ")") :
    ...
    if (SmackOptions::FloatEnabled) {
      ...
      for (auto is : IntOpGen::INTEGER_SIZES)
        if (!conv.bitCast || is == 8 ||
          std::find(FP_BIT_WIDTHS.begin(), FP_BIT_WIDTHS.end(), is) != FP_BIT_WIDTHS.end())
