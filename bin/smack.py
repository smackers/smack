...
def clang_frontend(args):
  ...
  p = subprocess.Popen(
    compile_command + [smack_lib, '-o', smack_bc],
    stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  clang_output = p.communicate()[0]

  if p.returncode:
    print >> sys.stderr, clang_output
    sys.exit("SMACK encountered an error when invoking clang. Exiting...")

  p = subprocess.Popen(
    compile_command + [args.input_file, '-o', args.bc_file],
    stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  clang_output = p.communicate()[0]
  ...
  p = subprocess.Popen(
    ['llvm-link', args.bc_file, smack_bc, '-o', args.bc_file],
    stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  linker_output = p.communicate()[0]

  if p.returncode:
    print >> sys.stderr, linker_output
...
def llvm_to_bpl(args):
  ...
  cmd = ['smack', args.bc_file]
  cmd.append('-source-loc-syms')
  cmd.append('-bpl=' + args.bpl_file)
  if args.debug: cmd.append('-debug')
  if "impls" in args.mem_mod: cmd.append('-mem-mod-impls')
  if args.bit_precise: cmd.append('-bit-precise')
  if args.bit_precise_pointers: cmd.append('-bit-precise-pointers')
  if args.no_byte_access_inference: cmd.append('-no-byte-access-inference')
  p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  smack_output = p.communicate()[0]

  if p.returncode:
    print >> sys.stderr, smack_output
...
def verify_bpl(args):
  ...
  try:
    ...
    verifier_output = p.communicate()[0]
  ...
  if p.returncode:
    print >> sys.stderr, verifier_output
