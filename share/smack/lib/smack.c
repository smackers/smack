...
void __SMACK_decls(void) {
  ...
  D("procedure $galloc(base_addr: ref, size: ref);\n"
    ...
    "ensures (forall q: ref :: {:qid \"smack.galloc.frame.size\"} {$Size[q]} "
    "q != base_addr ==> $Size[q] == old($Size[q]));\n"
    "ensures (forall q: ref :: {:qid \"smack.galloc.frame.alloc\"} {$Alloc[q]} "
    "q != base_addr ==> $Alloc[q] == old($Alloc[q]));\n");
  ...
  D("procedure $free(p: ref);\n"
    "modifies $Alloc;\n"
    "ensures !$Alloc[p];\n"
    "ensures (forall q: ref :: {:qid \"smack.free.frame.alloc\"} {$Alloc[q]} "
    "q != p ==> $Alloc[q] == old($Alloc[q]));\n");
