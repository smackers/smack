...
def reformat_assignment(line):
    ...
    # expSize = digit {digit}
    return re.sub(
        (r'((\d+)bv\d+|(-?)0x([0-9a-fA-F]+\.[0-9a-fA-F]+)e(-?)' r'(\d+)f(\d+)e(\d+))'),
        repl,
        line.strip(),
    )
...
def error_trace(verifier_output, verifier):
    ...
    from .cli.results import VResult

    traces = json_output(VResult.ERROR, verifier_output, verifier)['traces']
    output = '\n'.join(
        map(
            lambda trace: '{}({},{}): {}'.format(
                trace['file'], trace['line'], trace['column'], trace['description']
            ),
            traces,
        )
    )
...
def json_output(result, output, verifier, prettify=True):
    ...
    if result is VResult.VERIFIED:
        json_data = {'verifier': verifier, 'passed?': True}
        return json_data

    if verifier == 'boogie':
        traces = []
        traceP = re.compile(rf"(\s*)({FILENAME})\((\d+),\d+\):")
        steps = re.findall(traceP, output)
        for step in steps:
            if re.match('.*[.]bpl$', step[1]):
                line_no = int(step[2])
                with Path(step[1]).open() as f:
                    for ln in f.read().splitlines(True)[line_no : line_no + 10]:
                        src = re.match(rf".*{{:sourceloc \"({FILENAME})\", (\d+), (\d+)}}", ln)
                        if src:
                            traces.append(
                                {
                                    'file': src.group(1),
        ...
        traces = []
        traceP = re.compile(
            '(' + FILENAME + r')\((\d+),(\d+)\): Trace: Thread=(\d+)\s+\((.*(;\n)?.*)\)'
        )
        ...
        for step in raw_data:
            ...
            for token in description.split(','):
                ...
                if (
                    re.search(
                        (r'((CALL|RETURN from)\s+(\$|__SMACK))|' r'Done|ASSERTION'),
                        token,
                    )
                    is not None
                ):
    ...
    json_data = {
        ...
        'failsAt': failsAt,
        'threadCount': 1,
        'traces': (functools.reduce(merger, traces, []) if prettify else traces),
    }
    return json_data
