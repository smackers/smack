...
def svcomp_frontend(input_file, args):
    ...
    _name, _ext = Path(args.input_files[0]).stem, Path(args.input_files[0]).suffix
...
def svcomp_check_property(args):
    if args.svcomp_property:
        with Path(args.svcomp_property).open() as f:
            prop = f.read()
        from smack.top import VProperty, VResult
...
def verify_bpl_svcomp(args):
    ...
    corral_command += [f"/timeLimit:{time_limit}"]
    corral_command += ["/v:1"]
    ...
    if result in VResult.ERROR:  # normal inlining
        ...
        if 'Verifying program while tracking' in verifier_output:
            verifier_output = re.sub(
                re.compile('.*Verifying program while tracking', re.DOTALL),
...
def write_error_file(args, status, verifier_output):
    ...
    if args.error_file:
        ...
        if args.language == 'svcomp':
            error = smackJsonToXmlGraph(
                json_output_str(status, verifier_output, 'corral', False), args, hasBug, status
            )
        elif hasBug:
            error = smack.top.error_trace(verifier_output, 'corral')
