...
def scrub_pthreads(s):
  ...
  fltrs.append(r'typedef\s+union\s+{\s+struct\s+__pthread_mutex_s\s+__data;\s+char\s+__size\[\d+\];\s+long\s+int\s+__align;\s+}\s+pthread_mutex_t;')
  #pthread_cond_t
  ...
  fltrs.append(r'typedef\s+union\s+{\s+struct\s+__pthread_cond_s\s+__data;\s+char\s+__size\[\d+\];\s+__extension__\s+long\s+long\s+int\s+__align;\s+}\s+pthread_cond_t;')
  #pthread_condattr_t
