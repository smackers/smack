#! /usr/bin/env python3
# ruff: noqa: E501
# Hard-coded SVCOMP witness fixtures + XML/JSON sample blobs exceed line limit.
...
def addGraphNode(tree, data=None):
...
def addGraphEdge(tree, source, target, data=None):
...
def buildEmptyXmlGraph(args, hasBug):
    ...
    with Path(args.svcomp_property).open() as ppf:
        addKey(graph, "specification", ppf.read().strip())
    programfile = str(Path(args.orig_files[0]).resolve())
    addKey(graph, "programfile", programfile)
    with Path(programfile).open() as pgf:
        addKey(graph, "programhash", hashlib.sha256(pgf.read().encode('utf-8')).hexdigest())
    addKey(graph, "architecture", re.search(r'-m(32|64)', args.clang_options).group(1) + 'bit')
...
def formatAssign(assignStmt):
    ...
    if m:
        ...
        def repl(x):
            return '=' + x.group(1) + 'U' if x.group(2) is not None else ''
...
def isSMACKInitFunc(funcName):
    return (
        funcName == '$initialize'
        or funcName == '__SMACK_static_init'
        or funcName == '__SMACK_init_func_memory_model'
    )
        ...
        for jsonTrace in jsonTraces:
            ...
            if not pat.match(jsonTrace["file"]):
                ...
                if formattedAssign and formattedAssign.startswith('smack:ext:__VERIFIER_nondet'):
                    ...
                    attribs["assumption.scope"] = (
                        scope_func.split('.')[0] if '.' in scope_func else scope_func
                    )
                    addGraphEdge(tree, lastNode, newNode, attribs)
                    jsonTrace["line"]
                    jsonTrace["column"]
                ...
                if "CALL" in desc:
                    ...
                    if calledFunc.startswith("devirtbounce"):
                        print(
                            "Warning: calling function pointer dispatch procedure at line {}".format(
                                jsonTrace["line"]
                            )
                        )
                        continue
                ...
                if "RETURN from" in desc:
                    ...
                    if returnedFunc.startswith("devirtbounce"):
                        print(
                            "Warning: returning from function pointer dispatch procedure at line {}".format(
                                jsonTrace["line"]
                            )
                        )
                        continue
                    ...
                    if returnedFunc != callStack[-1][0]:
                        raise RuntimeError(
                            'Procedure Call/Return dismatch at line {}. Call stack head: {}, returning from: {}'.format(
                                jsonTrace["line"], callStack[-1][0], returnedFunc
                            )
                        )
                    callStack.pop()
    print()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        jsonStr = Path(sys.argv[1]).read_text()
